...
class JobRecord:
    ...
    def _timing_snapshot(self, now: float) -> dict[str, Any]:
        ...
        if (
            ...
            current_stage_elapsed = max(
                0.0, effective_now - self.active_stage_started_monotonic
            )
            durations[self.active_stage] = (
                durations.get(self.active_stage, 0.0) + current_stage_elapsed
            )
        return {
...
def _last_overall_percent(events: list[ProgressEvent]) -> int:
    if not events:
        return 0
    return events[-1].overall_percent
