...
class JobRecord:
    ...
    def _timing_snapshot(self, now: float) -> dict[str, Any]:
        ...
        if (
            ...
            current = max(
                0.0,
                effective_now - self.active_stage_started_monotonic,
            )
            durations[self.active_stage] = (
                durations.get(self.active_stage, 0.0) + current
            )
        return {
            "total_elapsed_seconds": round(
                max(0.0, effective_now - self.started_monotonic),
                3,
            ),
...
def render_heartbeat_line(job: JobRecord, width: int = 30) -> str:
    ...
    return (
        f"[연결 유지  ] [{bar}]     "
        f"(전체 {overall_percent:>3}%) | LLM 응답을 기다리는 중입니다. | "
        f"단계 누적 "
        f"{format_duration(timing['current_stage_elapsed_seconds'])} · "
    ...
    for stage in STAGE_ORDER:
        if stage in durations:
            lines.append(
                ...
                f"{format_duration(durations[stage])}\n"
            )
    lines.append(
        f"- 전체: {format_duration(timing['total_elapsed_seconds'])}\n"
    )
    return "".join(lines)
...
def format_duration(seconds: float) -> str:
    ...
    if hours:
        return f"{hours:02}:{minutes:02}:{remaining:04.1f}"
    return f"{minutes:02}:{remaining:04.1f}"
